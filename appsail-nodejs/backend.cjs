...
function createBackendRouter(options = {}) {
  ...
  router.post("/webhook/whatsapp", async (req, res) => {
    try {
      ...
      console.log(`[Webhook] Extraction results - Incoming: ${incoming.length}, Statuses: ${statuses.length}`);
      ...
      if (incoming.length > 0) {
        ...
        console.log(`[Webhook] SalesIQ Screen Name: ${screenName || "NOT SET"}`);
        
        if (screenName) {
          console.log(`[Webhook] Routing ${incoming.length} messages to SalesIQ...`);
          await Promise.allSettled(
            incoming.map((message) => routeToSalesIQ(message))
          );
        }
  ...
  // SalesIQ Outgoing Webhook
  router.post("/webhook/salesiq", async (req, res) => {
    try {
      console.log("[SalesIQ Webhook] Received payload:", JSON.stringify(req.body));
      
      // SalesIQ outgoing webhook payload structure:
      // {
      //   "event": "message_sent",
      //   "data": {
      //     "message": { "text": "...", ... },
      ...
      const visitorData = req.body.data?.visitor;
      
      if (event === "message_sent" && messageData && visitorData) {
        const phone = normalizePhone(visitorData.phone);
        const text = messageData.text;
        
        if (phone && text) {
          console.log(`[SalesIQ Webhook] Routing outgoing message to ${phone}`);
          ...
          if (sendConfig.enabled) {
            const responseBody = await postJson(sendConfig.url, payload, sendConfig.headers);
            console.log(`[SalesIQ Webhook] Outbound response:`, JSON.stringify(responseBody));
            ...
            console.error(`[SalesIQ Webhook] Outbound provider not enabled`);
          }
        }
      }
      ...
      console.error("[SalesIQ Webhook] Error processing payload:", error.message);
      res.status(500).json({
        error: "Failed to process SalesIQ webhook",
        message: error.message
      });
    }
  });
...
function getWebhookValueObjects(payload) {
  ...
  // Handle direct value object (common in some 360dialog/Meta versions)
  if (payload && payload.value && typeof payload.value === "object") {
    values.push(payload.value);
  }

  // Handle flat structure (common in some 360dialog versions)
  if (
    payload &&
    (
  ...
  // Handle Meta direct structure if entry is missing
  if (payload && payload.object === "whatsapp_business_account" && !payload.entry) {
    console.log("[Webhook] Detected Meta structure without entry array");
  }
...
async function routeToSalesIQ(message) {
  const screenName = process.env.SALESIQ_SCREEN_NAME;
  if (!screenName) {
    console.log("[SalesIQ] SALESIQ_SCREEN_NAME not set, skipping routing");
    return;
  }

  try {
    console.log(`[SalesIQ] Routing message from ${message.phone} to SalesIQ (Screen: ${screenName})`);
    
    const tokenInfo = await getZohoAccessToken();
    const serverUri = process.env.ZOHO_SALESIQ_SERVER_URI || "salesiq.zoho.com";
    
    // For Custom Messaging Channels, the endpoint is typically:
    // https://salesiq.zoho.com/api/v2/{screen_name}/external/messages
    // The payload requires specific fields for the external channel integration.
    ...
    const payload = {
      ...
      visitor: {
        ...
        phone: message.phone,
        id: message.phone // Use phone as unique external ID
      },
      external_id: message.metaMessageId || message.id,
      timestamp: new Date(message.timestamp).getTime()
    };

    const endpoint = `https://${serverUri}/api/v2/${screenName}/external/messages`;
    
    console.log(`[SalesIQ] Pushing to: ${endpoint}`);
    console.log(`[SalesIQ] Payload: ${JSON.stringify(payload)}`);
    ...
    console.log(`[SalesIQ] Response: ${JSON.stringify(response)}`);
    console.log(`[SalesIQ] Successfully routed message to SalesIQ`);
  } catch (error) {
    console.error(`[SalesIQ] Error routing to SalesIQ: ${error.message}`);
    if (error.details) {
      console.error(`[SalesIQ] Error details: ${JSON.stringify(error.details)}`);
    }
  }
}
